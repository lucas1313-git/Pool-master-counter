(function () {
  ...
  // Pro unlock (one-time in-app purchase). Free tier: individual mode
  // only, a single game type (no rotation), no tournaments, no player
  // stats/ratings pages, no day reports, and a capped roster. See
  // requirePro() / the pro-upsell-overlay wiring near the bottom of this
  // file for every gate point, and MOBILE_APP_SETUP.md for how the real
  // purchase (RevenueCat) plugs in on the native side.
  var PRO_KEY = "poolMasterCounter.pro.v1";
  var FREE_PLAYER_CAP = 4;
  var isPro = loadProFromStorage();
  ...
  // ---------------------------------------------------------------------
  // Pro unlock overlay + purchase flow
  // ---------------------------------------------------------------------

  var proUpsellOverlay = document.getElementById("pro-upsell-overlay");
  var proUpsellMessage = document.getElementById("pro-upsell-message");
  var btnProUpsellBuy = document.getElementById("btn-pro-upsell-buy");
  var btnProUpsellRestore = document.getElementById("btn-pro-upsell-restore");
  var btnProUpsellCancel = document.getElementById("btn-pro-upsell-cancel");

  function isNativeApp() {
    return !!(window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform());
  }

  // Replace with the real public SDK key from the RevenueCat dashboard
  // (Project Settings > API Keys > Apple App Store) before shipping - see
  // MOBILE_APP_SETUP.md. This is a public, client-side key by design
  // (RevenueCat's own docs treat it like one - it can only be used to
  // make purchases and read entitlements for the signed-in App Store
  // account, not to grant entitlements or move money on its own).
  var REVENUECAT_IOS_API_KEY = "appl_REPLACE_ME";

  // Configures the RevenueCat SDK and reconciles isPro with the App
  // Store account's actual entitlement state - catches a purchase made
  // on another device under the same Apple ID, or a refund, rather than
  // trusting only the locally-cached flag. No-ops in the browser (there's
  // no native Purchases plugin there), so isPro just stays whatever
  // dev-mode purchasePro()/localStorage last set it to.
  function initRevenueCatIfNative() {
    if (!isNativeApp() || !window.Purchases) return;
    window.Purchases.configure({ apiKey: REVENUECAT_IOS_API_KEY })
      .then(function () {
        return window.Purchases.getCustomerInfo();
      })
      .then(function (result) {
        var active = result && result.customerInfo && result.customerInfo.entitlements && result.customerInfo.entitlements.active;
        setProUnlocked(!!(active && active.pro));
      })
      .catch(function (e) {
  ...
  function setProUnlocked(unlocked) {
    isPro = unlocked;
    try {
      localStorage.setItem(PRO_KEY, unlocked ? "1" : "0");
    } catch (e) {}
    renderAll();
  }

  function openProUpsell(messageKey, messageVars) {
    proUpsellMessage.textContent = T(messageKey, messageVars);
    proUpsellOverlay.classList.remove("hidden");
  }

  function closeProUpsell() {
    proUpsellOverlay.classList.add("hidden");
  }

  // Every locked feature's entry point calls this instead of running
  // directly. Pro users pass straight through; everyone else sees the
  // upsell instead of the feature (never a half-broken locked state).
  function requirePro(messageKey, onProceed, messageVars) {
    if (isPro) {
      if (onProceed) onProceed();
      return true;
    }
    openProUpsell(messageKey, messageVars);
    return false;
  }

  // Real purchase on-device: RevenueCat's Capacitor plugin (window.Purchases,
  // installed via `npm install @revenuecat/purchases-capacitor` and
  // configured per MOBILE_APP_SETUP.md) fetches the current offering and
  // buys its package, then unlocks Pro once the "pro" entitlement comes
  // back active. In the browser (this dev server, no native shell) there's
  // no real store to buy from, so it just flips the flag locally - lets
  // every Pro-gated feature be exercised and tested without a device.
  function purchasePro() {
    if (!isNativeApp()) {
      setProUnlocked(true);
      closeProUpsell();
      showToast("Pro unlocked (dev mode - no real purchase made)");
      return;
    }
    if (!window.Purchases) {
      alertModal(T("pro.purchaseUnavailable"));
      return;
    }
    window.Purchases.getOfferings()
      .then(function (offerings) {
        var current = offerings.current;
        var pkg = current && current.availablePackages && current.availablePackages[0];
        if (!pkg) throw new Error("no-package");
        return window.Purchases.purchasePackage({ aPackage: pkg });
      })
      .then(function (result) {
        var active = result && result.customerInfo && result.customerInfo.entitlements && result.customerInfo.entitlements.active;
        if (active && active.pro) {
          setProUnlocked(true);
          closeProUpsell();
          showToast(T("pro.purchaseSuccess"));
        }
      })
      .catch(function (e) {
        if (e && e.userCancelled) return;
        alertModal(T("pro.purchaseFailed"));
      });
  }

  function restorePro() {
    if (!isNativeApp()) {
      showToast("Dev mode - nothing to restore here, use Unlock Pro instead.");
      return;
    }
    if (!window.Purchases) {
      alertModal(T("pro.purchaseUnavailable"));
    ...
    window.Purchases.restorePurchases()
      .then(function (result) {
        var active = result && result.customerInfo && result.customerInfo.entitlements && result.customerInfo.entitlements.active;
        if (active && active.pro) {
          setProUnlocked(true);
  ...
  btnProUpsellBuy.addEventListener("click", purchasePro);
  btnProUpsellRestore.addEventListener("click", restorePro);
  btnProUpsellCancel.addEventListener("click", closeProUpsell);
  proUpsellOverlay.addEventListener("click", function (e) {
    if (e.target === proUpsellOverlay) closeProUpsell();
  });
  ...
  function addPlayer(name, startingRating) {
    ...
    if (!name) return null;
    if (!isPro && state.players.length >= FREE_PLAYER_CAP) {
      requirePro("pro.featureRosterCap", null, { cap: FREE_PLAYER_CAP });
      return null;
    }
    var player = {
      id: uid(),
  ...
  function openPlayerStatsPage(name, skipHistory) {
    if (!name) return;
    if (!requirePro("pro.featureStats")) return;
  ...
  function openTournamentPage(skipHistory) {
    if (!requirePro("pro.featureTournament")) return;
  ...
  Array.prototype.forEach.call(modeRadios, function (radio) {
    radio.addEventListener("change", function () {
      ...
      if (radio.value === "teams" && !isPro) {
        Array.prototype.forEach.call(modeRadios, function (r) {
          r.checked = r.value === "individual";
        });
        requirePro("pro.featureTeams");
        return;
      }
  ...
  rotationEnabledCheckbox.addEventListener("change", function () {
    if (rotationEnabledCheckbox.checked && !isPro) {
      rotationEnabledCheckbox.checked = false;
      requirePro("pro.featureRotation");
      return;
    }
  ...
  btnDayReportCopy.addEventListener("click", function () {
    requirePro("pro.featureReports", function () {
